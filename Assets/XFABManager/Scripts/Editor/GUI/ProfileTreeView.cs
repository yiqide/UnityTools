...
namespace XFABManager
{
    public class ProfileTreeView : TreeView
    {
        ...
        private static MultiColumnHeaderState.Column[] GetColumns()
        {
            ...
            retVal[5].headerContent = new GUIContent("ProjectName", "项目名,当前这个配置所作用的项目!");
            retVal[5].minWidth = 30;
            retVal[5].width = 200;
            retVal[5].maxWidth = 1000;
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            for (int i = 0; i < XFABManagerSettings.Settings.Groups.Count; i++)
            {
                string group = XFABManagerSettings.Settings.Groups[i];
                TreeViewItem groupItem = new TreeViewItem(group.GetHashCode(), 0, group);

                var profiles = XFABManagerSettings.Settings.Profiles.Where(x => x.GroupName.Equals(group));
        ...
        private void CellGUI(Rect cellRect, TreeViewItem item, int column, ref RowGUIArgs args)
        {
            ...
            if (item.depth == 0)
            {
                ...
                // Profile 
                Profile profile = XFABManagerSettings.Settings.Profiles.Where(a => a.name.Equals(item.displayName) && a.GroupName.Equals(item.parent.displayName)).FirstOrDefault();
        ...
        protected override void RenameEnded(RenameEndedArgs args)
        {
            ...
            if (args.newName.Length > 0)
            {
                if (!args.newName.Equals(args.originalName))
                {
                    ...
                    {
                        // Group
                        if (XFABManagerSettings.Settings.Groups.Contains(args.newName))
                        {
                            Debug.LogWarningFormat("重命名失败!名称{0}已经存在!", args.newName);
                            return;
                        }

                        int index = XFABManagerSettings.Settings.Groups.IndexOf(args.originalName);
                        XFABManagerSettings.Settings.Groups[index] = args.newName;

                        // 修改Profile 
                        var profiles = XFABManagerSettings.Settings.Profiles.Where(x => x.GroupName.Equals(args.originalName));
                    ...
                    else if(item.depth == 1)
                    {
                        // Profile
                        // 新名称已经存在
                        if (XFABManagerSettings.Settings.IsContainsProfileName(args.newName, item.parent.displayName))
                        {
                            Debug.LogWarningFormat("重命名失败!名称{0}已经存在!", args.newName);
                            return;
                        }
        ...
        void DeleteBundle(object id)
        {
            ...
            if ( item != null )
            {
                if (item.depth == 0) {
                    // 删除 Group 
                    XFABManagerSettings.Settings.Groups.RemoveAll(x => x.Equals(item.displayName));
                    XFABManagerSettings.Settings.Profiles.RemoveAll(x => x.GroupName.Equals(item.displayName));
                } else if (item.depth == 1) { 
                    // 删除 Profile
        ...
        void AddNewGroup(object id) {
            ...
            XFABManagerSettings.Settings.Groups.Add(name);
