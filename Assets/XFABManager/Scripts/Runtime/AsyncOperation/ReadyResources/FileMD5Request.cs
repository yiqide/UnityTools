...
public class FileMD5Request : CustomAsyncOperation<FileMD5Request>
{
    ...
    private void AddFileMd5Cache(string file,string md5) {
        if (md5_caches.ContainsKey(file))
            md5_caches[file] = md5;
        else
            md5_caches.Add(file, md5);

        if (md5_caches.Count > md5_cache_file_count)
            md5_caches.Clear();

    }

    private string GetFileMd5Cache(string file) {
        string md5 = string.Empty;
        md5_caches.TryGetValue(file, out md5);
        return md5;
    }
